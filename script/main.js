$(function(){
    ...
    $(window).scroll(function(){
        ...
        if(scrollTop<mbiTop6-200 && scrollTop>mbiTop5-200){
                $(`.mid_banner_img6`).removeClass('fix');
                $(`.mid_banner_img5`).addClass('fix');
                $('.text5').css('opacity','1');
                $('.mid_banner_controls li').removeClass('long');
                $('.mid_banner_controls li').eq(4).addClass('long');
        }else if(scrollTop>=mbiTop6-200){  // 스크롤에 따른 about의 animate
        ...
        if(scrollTop>eventTop-200){  // 스크롤에 따른 about의 animate
            if($('#event').attr('data-ok')!='ok'){
                ...
                if($(window).width()>1200){
                    $('.event_img').css('border-radius','300px').css('height', '60vh').css('max-width', '75%').css('margin', '20vh auto');
                    $('.event_logo').css('top','15%');
                }else if($(window).width()>800){
                    $('.event_img').css('border-radius','300px').css('height', '50vh').css('max-width', '90%').css('margin', '20vh auto');
                    $('.event_logo').css('top','8%');
                }else{
                    //이미지 새로 만들기
                    $('.event_img').css('border-radius','300px').css('height', '70vh').css('max-width', '90%').css('margin', '20vh auto').css('background-position','right');
                    $('.event_logo').css('top','17%');
                }
                setTimeout(function(){
                if($(window).width()>800){
                    $('.mid_line').css('width','300px');
                }else{
                    $('.mid_line').css('width','80px');
                }
                },500)
            }
        }
